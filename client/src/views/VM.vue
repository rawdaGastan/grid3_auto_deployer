		...
		<v-row justify="center">
			<v-col cols="12" sm="6">
				<v-form v-model="verify" ref="form" @submit.prevent="deployVm">
					<v-text-field label="Name" :rules="nameValidation" class="my-2" v-model="name" bg-color="accent"
					...
					<BaseButton type="submit" block class="bg-primary" :loading="loading" :disabled="!verify || alert"
						text="Deploy" />
				</v-form>
			</v-col>
		</v-row>
						...
						<v-data-table :headers="headers" :items="results" class="elevation-1">
							<template v-slot:item="{ item }">
								<tr>
									...
									<td v-if="item.raw.public_ip" class="cursor-pointer" @click="copyIP(item.raw.public_ip)">
										{{ item.raw.public_ip }}
									</td>
									<td v-else>-</td>
									<td>
										<font-awesome-icon v-if="!item.raw.deleting" class="text-red-accent-2 cursor-pointer"
											@click="deleteVm(item.raw);" icon="fa-solid fa-trash" />
