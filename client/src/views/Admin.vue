				...
				<v-data-table v-model:items-per-page="itemsPerPage" :headers="vouchersHeaders" :items="vouchers"
					...
					<template v-slot:item="{ item }">
						<tr>
							...
							<td v-if="!dateIsNull(item.columns.updated_at)">{{ stringDate(item.columns.updated_at) }}</td>
							<td v-else>-</td>
				...
				<v-data-table v-model:items-per-page="itemsPerPage" :headers="pendingVouchersHeaders" :items="pendingVouchers"
					...
					<template v-slot:item="{ item }">
						<tr>
							...
							<td v-if="!dateIsNull(item.columns.created_at)">{{ stringDate(item.columns.created_at) }}</td>
							<td v-else>-</td>
							<td v-if="item.raw.reason">{{ item.raw.reason }}</td>
							<td v-else>-</td>
							<td>{{ item.raw.vms }}</td>
							<td>{{ item.raw.public_ips }}</td>
								...
								<v-row>
									<v-col cols="6">
										<BaseButton color="primary" text="Approve" size="small" class="mr-1 px-8 text-capitalize"
											variant="flat" @click="approveVoucher(item.raw.id, true)" />
									</v-col>
									<v-col cols="6">
										<BaseButton color="red-lighten-1" text="Reject" size="small" class="text-capitalize" variant="flat"
											@click="approveVoucher(item.raw.id, false)" />
									</v-col>
								</v-row>
							</td>
						</tr>
