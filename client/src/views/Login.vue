			...
			<v-col>
				<v-hover v-slot="{ isHovering, props }" open-delay="200">
					<v-img :style="isHovering
						...
						" transition="transform .2s" contain height="600" src="@/assets/login.png" :class="{ 'on-hover': isHovering }"
						v-bind="props" />
				</v-hover>
			</v-col>
			...
			<v-col>
				...
				<v-form v-model="verify" @submit.prevent="onSubmit">
					...
					<v-text-field v-model="password" :rules="rules" clearable label="Password" placeholder="Enter your password"
						...
						style="grid-area: unset;" density="compact"></v-text-field>

					<div class="text-body-2 text-end">
						<router-link class="text-body-2 text-decoration-none primary" to="/forgetPassword">Forget
							password?</router-link>
					</div>

					<div class="text-body-2 mb-n1 text-center">
						<v-btn color="primary" rel="noopener noreferrer" type="submit" class="w-100 d-block my-5" :disabled="!verify"
							:loading="loading" variant="flat">
							Sign in
						</v-btn>
						<p>
							Don't have an account?
							<router-link class="text-body-2 text-decoration-none primary" to="/signup">Sign up</router-link>
						</p>
					</div>
				</v-form>
			</v-col>
