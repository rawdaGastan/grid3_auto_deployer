	...
	<v-container>
		...
		<v-row justify="center">
			<v-col cols="12" sm="6">
				<v-form v-model="verify" ref="form" @submit.prevent="deployK8s">
					<v-text-field label="Name" :rules="nameValidation" class="my-2" v-model="k8Name" bg-color="accent"
						variant="outlined" density="compact"></v-text-field>
					<BaseSelect :value="selectedResources" placeholder="Resources" :modelValue="selectedResources"
						:items="resources" :rules="rules" class="mt-3" @update:modelValue="selectedResources = $event" />
					...
					<v-dialog transition="dialog-top-transition" max-width="500">
						<template v-slot:activator="{ props }">
							<div class="mx-auto d-flex justify-center">
								<BaseButton type="submit" :disabled="!verify || alert" class="w-25 d-inline-block bg-primary mr-2"
									:loading="loading" text="Deploy" />
								<BaseButton color="primary" class="w-25 d-inline-block" v-bind="props" icon="fa-plus" text="workers" />
							</div>
						</template>
							...
							<v-card width="100%" size="100%" class="mx-auto pa-5">
								<v-form ref="wForm" v-model="listVerify">
									<v-card-text>
										...
										<v-form v-model="workerVerify" @submit.prevent="addWorker" v-if="showInputs">
											<v-text-field label="Name" bg-color="accent" variant="outlined" v-model="workerName"
												density="compact" :rules="nameValidation"></v-text-field>
											<BaseSelect :value="workerSelResources" placeholder="Resources" :modelValue="workerSelResources"
												:items="workerResources" :rules="rules" class="my-3"
												@update:modelValue="workerSelResources = $event" />
											<v-btn type="submit" :disabled="!workerVerify" density="comfortable"
												class="bg-primary d-flex ml-auto">Add</v-btn>
										</v-form>
										<h3 v-if="savedWorkers.length > 0">Saved workers</h3>
										<v-list density="compact" v-if="savedWorkers.length > 0">
											<v-list-item v-for="(worker, i) in savedWorkers" :key="i" :value="worker">
												...
												<template v-slot:append>
													<font-awesome-icon class="primary pointer" icon="fa-solid fa-xmark"
														@click="deleteWorker(worker.name)" />
												</template>
												<v-list-item-action></v-list-item-action>
											</v-list-item>
										</v-list>
										<v-btn variant="text" v-if="!showInputs" @click="showInputs = true"
									...
									<v-card-actions class="justify-center">
										<BaseButton class="bg-primary mr-5" @click="isActive.value = false" text="Cancel" />
										<BaseButton :disabled="savedWorkers.length == 0" class="bg-primary" @click="isActive.value = false"
											text="Save" />
									</v-card-actions>
								</v-form>
							</v-card>
		...
		<v-row v-if="results.length > 0">
			<v-col class="d-flex justify-end">
				<BaseButton color="red-accent-2" :loading="deLoading" @click="deleteAllK8s" text="Delete All" />
			</v-col>
		</v-row>
		<v-row v-if="results.length > 0">
			<v-col>
				<v-data-table :headers="headers" :items="results" class="elevation-1">
					<template v-slot:item="{ item }">
						<tr>
							...
							<td class="cursor-pointer" @click="copyIP(item.raw.master.ygg_ip)">
								{{ item.raw.master.ygg_ip }}
							</td>
							<td v-if="item.raw.master.public_ip" class="cursor-pointer" @click="copyIP(item.raw.master.public_ip)">
								{{ item.raw.master.public_ip }}
							</td>
							<td v-else>-</td>
							<td>
								<font-awesome-icon v-if="!item.raw.deleting" class="text-red-accent-2 mr-5 cursor-pointer" @click="
									deleteK8s(item.raw)
									" icon="fa-solid fa-trash" />
								<v-progress-circular v-else indeterminate color="red" size="20"
									style="margin-right:5px"></v-progress-circular>
								<font-awesome-icon v-if="item.raw.workers.length > 0" class="text-primary cursor-pointer"
									icon="fa-solid fa-eye" @click="displayWorkers(item.raw.workers)" />
							</td>
						</tr>
					</template>
				</v-data-table>
				<v-dialog transition="dialog-top-transition" v-model="dialog">
					<v-card width="50%" class="mx-auto">
						...
						<v-card-text>
							<h5 class="text-h5 text-md-h4 font-weight-bold text-center my-5 secondary">
								Workers
							</h5>
							<v-data-table :headers="workerHeaders" :items="workers" class="elevation-1">
							</v-data-table>
						</v-card-text>
					</v-card>
				</v-dialog>
	...
	setup() {
		...
		const nameValidation = ref([
			(value) => {
				...
				if (!/^[a-z]+$/.test(value)) return "Name can only include lowercase alphanumeric characters";
				return true;
			},
