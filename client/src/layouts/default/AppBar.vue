			...
			<v-container class="d-flex">
				<v-app-bar-title>
					<router-link to="/" @click="setActive(0, '/')">
						<v-img src="@/assets/codescalers.png" height="100%" width="150px" class="mt-3 mt-md-5" />
					</router-link>
				</v-app-bar-title>
				...
				<v-list class="hidden-md-and-down">
					<v-list-item-title class="py-3">
						<router-link v-for="(item, index) in items" :key="index" :to="item.path"
							class="pa-5 primary text-decoration-none" @click="setActive(index, item.title)"
				...
				<v-menu v-if="user.length != 0">
					<template v-slot:activator="{ props }">
						<v-btn class="primary ml-1 mt-2 pt-0 mt-md-3 text-capitalize" v-bind="props"
					...
					<v-list>
						<v-list-item>
							<v-list-item-title>
								<router-link v-for="item in user" :key="item.title" :to="item.path"
				...
				<v-menu id="notifications" v-if="user.length != 0">
					<template v-slot:activator="{ props }">
						<v-btn class="primary ml-1 mt-2 pt-0 mt-md-3 text-capitalize" v-bind="props">
							<v-badge v-if="notifications.length > 0" color="#5CBBF6" :content="notifications.length">
					...
					<v-list max-height="400px" v-if="notifications.length > 0" density="compact">
				...
				<v-app-bar-nav-icon class="primary hidden-md-and-up" @click.stop="drawer = !drawer"></v-app-bar-nav-icon>
			</v-container>
		</v-app-bar>
